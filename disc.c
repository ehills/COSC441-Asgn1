...
disk_read(monitor *read_monitor, disc_container *this)
{
    ...
    fprintf(stderr,"\nREAD\tblock: %ld\tbuffer: %p\trequest time: %ld\tcompletion time: %ld\n\n", block_number, buffer_address, read_monitor->request_time, this->disc_time);
    return 0;
}
...
disk_write(monitor *write_monitor, disc_container *this)
{
    ...
    fprintf(stderr,"\nWRITE\tblock: %ld\tbuffer: %p\trequest time: %ld\tcompletion time: %ld\n\n", block_number, buffer_address, write_monitor->request_time, this->disc_time);
