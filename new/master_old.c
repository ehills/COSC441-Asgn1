...
int is_cb_full(circular_buffer cbuf) {
    return cbuf.end == (cbuf.start -1) % CBUF_SIZE;
}
...
int cbuffer_add(job *entry, disc_container *disc) {
    ...
    fprintf(stdout, "Write to queue pos: %d\n", disc->cbuf.end);

    disc->cbuf.jobs[disc->cbuf.end] = entry;
    disc->cbuf.end = (disc->cbuf.end +1) % CBUF_SIZE;

    fprintf(stdout, "NEXT Write to queue pos: %d\n", disc->cbuf.end);
...
main(int argc, char **argv) 
{
    ...
    for(j=0; j < num_disks; j++) {
        ...
        fprintf(stdout, "Start job: %d end job: %d\n", discs[j].cbuf.jobs[discs[j].cbuf.start]->message, discs[j].cbuf.jobs[discs[j].cbuf.start]->message);
    }
